...
calculado AS (
    SELECT
        ...
        dev.status,
        dev.cas,
        dev.protocolo_secretaria,
        dev.protocolo_id,
        dev.protocolo_descricao,
        ...
        dev.telefone_cadun,
        dev.bairro_cadun,
        dev.unidade_territorial_cadun,
        dev.endereco_cadun,
        dev.complemento_cadun,
        dev.complemento_adicional_cadun,
    ...
    FROM {{ cw_table }} AS cw
    INNER JOIN {{ dev_table }} AS dev
        ON cw.id_membro_familia = dev.id_membro_familia
    WHERE dev.data_particao = (SELECT MAX(t.data_particao) FROM {{ dev_table }} AS t)
),
...
SELECT calculado.*
FROM calculado
INNER JOIN atual ON CAST(calculado.objectid AS STRING) = atual.objectid
WHERE
