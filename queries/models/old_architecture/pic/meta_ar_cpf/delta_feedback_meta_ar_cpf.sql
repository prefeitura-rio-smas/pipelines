...
WITH
    calculado AS (
        SELECT
            ...
            dev.flag_ativo, dev.flag_atual, dev.numero_entrada,
...
WHERE
    ...
    OR COALESCE(NULLIF(calculado.flag_ativo, 'None'), '')            != COALESCE(NULLIF(atual.flag_ativo, 'None'), '')
    OR COALESCE(NULLIF(calculado.flag_atual, 'None'), '')            != COALESCE(NULLIF(atual.flag_atual, 'None'), '')
